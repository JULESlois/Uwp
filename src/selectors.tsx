import { forwardRef, useEffect, useId, useRef, type FieldsetHTMLAttributes, type ForwardedRef, type InputHTMLAttributes, type ReactElement, type ReactNode } from 'react'
...
export const CheckBox = forwardRef<HTMLInputElement, CheckBoxProps>(function CheckBox({ checked, onChange, label, ariaLabel, stopPropagation = false, disabled = false, indeterminate = false, className = '', 'aria-label': nativeAriaLabel, ...inputProps }, forwardedRef) {
...
export const RadioButton = forwardRef<HTMLInputElement, RadioButtonProps>(function RadioButton({ checked, onChange, label, ariaLabel, stopPropagation = false, disabled = false, className = '', 'aria-label': nativeAriaLabel, ...inputProps }, ref) {
...
export type RadioGroupOption<T extends string> = { value: T; label: ReactNode; disabled?: boolean }
...
function RadioGroupInner<T extends string>({ value, onChange, options, legend, name, orientation = 'vertical', disabled = false, required = false, className = '', ...fieldsetProps }: RadioGroupProps<T>, ref: ForwardedRef<HTMLFieldSetElement>) {
  const generatedName = useId()
  const groupName = name ?? generatedName
  return <fieldset {...fieldsetProps} ref={ref} disabled={disabled} className={`radio-group ${orientation}${className ? ` ${className}` : ''}`}>
    {legend != null && <legend>{legend}</legend>}
    {options.map((option) => <RadioButton key={option.value} name={groupName} value={option.value} checked={option.value === value} disabled={option.disabled} required={required} onChange={(checked) => { if (checked) onChange(option.value) }} label={option.label} />)}
  </fieldset>
}

export const RadioGroup = forwardRef(RadioGroupInner) as <T extends string>(props: RadioGroupProps<T> & { ref?: ForwardedRef<HTMLFieldSetElement> }) => ReactElement
...
export const ToggleSwitch = forwardRef<HTMLInputElement, ToggleSwitchProps>(function ToggleSwitch({ checked, onChange, label, detail, disabled = false, className = '', ...inputProps }, ref) {
