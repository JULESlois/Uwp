...
function PatternsPage({ era }: { era: Era }) {
  ...
  return <section className="page-enter"><header className="section-head"><h2>交互模式</h2><p>临时界面继续遵循键盘焦点约束；窗口级命令由 CommandBar 的主/次优先级决定何时进入 overflow。</p></header><div className="pattern-grid expanded"><article><h3>Flyout</h3><p>锚定触发位置，键盘焦点进入菜单后不会丢失上下文。</p><Flyout open={flyout} onClose={() => setFlyout(false)} anchor={<Button onClick={() => setFlyout((value) => !value)}>打开 Flyout</Button>}><button role="menuitem" onClick={() => setFlyout(false)}>固定到开始屏幕</button><button role="menuitem" onClick={() => setFlyout(false)}>添加到收藏</button><button role="menuitem" disabled>系统保留</button><button role="menuitem" onClick={() => setFlyout(false)}>属性</button></Flyout></article><article><h3>ContentDialog</h3><p>只用于必须明确确认后才能继续的流程，Tab 焦点被限制在对话框内部。</p><Button variant="accent" onClick={() => setDialog(true)}>打开对话框</Button></article><article><h3>ContextMenu</h3><p>鼠标右键或 Shift + F10 都可以打开上下文命令。</p><ContextMenu items={[{ label: '打开', onClick: () => setStatus('已打开项目') }, { label: '重命名', onClick: () => setStatus('进入重命名') }, { label: '删除', onClick: () => setStatus('已执行删除示例') }, { label: '系统保留命令', disabled: true }]}><div className="context-target">右键或 Shift + F10</div></ContextMenu></article><article><h3>TeachingTip</h3><p>用于就地解释新功能，不阻断当前任务。</p><TeachingTip open={tip} title="Semantic Zoom" onClose={() => setTip(false)} anchor={<Button onClick={() => setTip(true)}>显示提示</Button>}><p>使用缩放按钮在磁贴与分组概览之间切换。</p></TeachingTip></article><article><h3>SettingsPane</h3><p>Windows 8 从屏幕边缘进入；Windows 10 可作为窄设置窗格。</p><Button onClick={() => setSettings(true)}>打开设置面板</Button></article><article><h3>Snap View</h3><p>布局变化不是简单缩放，而是重新选择保留的信息与操作。</p></article></div><SnapView snapped={snapped} onChange={setSnapped} /><div className="appbar-demo"><div><h3>AppBar / BottomAppBar</h3><p>{status}</p></div><AppBar commands={[{ label: '添加', glyph: '+', onClick: () => setStatus('已添加项目') }, { label: '固定', glyph: '⌖', onClick: () => setStatus('已固定到开始屏幕') }, { label: '删除', glyph: '×', onClick: () => setStatus('已删除项目') }, { label: '不可用', glyph: '·', disabled: true }]} /></div><div className="material-demo"><RevealSurface><AcrylicPane><h3>{era === 'win10' ? 'Acrylic + Reveal' : '平面 Surface'}</h3><p>{era === 'win10' ? 'Reveal 只保留低强度边缘掠光；高对比度或 forced-colors 环境下会自动关闭材质效果。' : 'Windows 8 模式关闭 Acrylic 与 Reveal，保留纯色平面和清晰层级。'}</p></AcrylicPane></RevealSurface></div><ContentDialog open={dialog} title="保存更改？" primaryButtonText="保存" secondaryButtonText="取消" onPrimaryButtonClick={() => setStatus('已保存更改')} onClose={() => setDialog(false)}><p>这是阻断式确认示例；主操作与次操作由 ContentDialog API 配置。</p></ContentDialog><SettingsPane open={settings} title="当前应用设置" onClose={() => setSettings(false)}><div className="pane-settings"><ToggleSwitch checked={true} onChange={() => undefined} label="允许通知" /><ToggleSwitch checked={false} onChange={() => undefined} label="后台更新" /><button className="pane-link" onClick={() => setSettings(false)}>权限与隐私</button></div></SettingsPane></section>
}
