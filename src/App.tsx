...
import { SelectionGridView, SelectionListView, type SelectionMode } from './collections'
...
function ViewsPage() {
  const [items, setItems] = useState<ListItem[]>(libraryItems)
  ...
  return <section className="page-enter"><header className="section-head"><h2>列表与分栏</h2><p>选择与打开项目现在分离：Space/单击负责 selection，Enter/双击负责 invoke；Extended 模式同时支持 Ctrl/⌘ 离散选择与 Shift 范围选择。</p></header><div className="collection-toolbar"><ComboBox label="SelectionMode" value={selectionMode} onChange={changeMode} options={[{ value: 'none', label: 'None · 不选择' }, { value: 'single', label: 'Single · 单选' }, { value: 'multiple', label: 'Multiple · 独立多选' }, { value: 'extended', label: 'Extended · Ctrl / Shift' }]} /><p className="collection-status">最近打开：{invoked}</p></div><div className="view-grid"><article className="view-block"><div className="view-heading"><h3>ListView</h3><strong>已选择 {selected.length} 项</strong></div><SelectionListView items={items} selected={selected} onSelectionChange={setSelected} selectionMode={selectionMode} onItemInvoked={(item) => setInvoked(item.title)} reorderable onReorder={setItems} /><p className="collection-help">拖动可重排；键盘使用 Alt + ↑/↓。不可用项目不会进入选择或重排。</p></article><article className="view-block"><div className="view-heading"><h3>GridView</h3><strong>已选择 {gridSelected.length} 项</strong></div><SelectionGridView items={items} selected={gridSelected} onSelectionChange={setGridSelected} selectionMode={selectionMode} onItemInvoked={(item) => setInvoked(item.title)} /></article><article className="view-block wide"><div className="view-heading"><h3>SplitView</h3></div><SplitView pane={<nav className="folder-list">{folders.map((item) => <RadioButton key={item.key} name="folder" value={item.key} checked={folder === item.key} onChange={() => setFolder(item.key)} label={item.label} />)}</nav>}><div className="split-detail"><h3>{folders.find((item) => item.key === folder)?.label}</h3><p>分栏仍直接复用基础 RadioButton，集合视图则按 SelectionMode 在 RadioButton 与 CheckBox 之间切换。</p><div className="message-lines"><span /><span /><span /></div></div></SplitView></article><article className="view-block wide"><div className="view-heading"><h3>MasterDetailsView</h3></div><MasterDetailsView items={items} value={master} onChange={setMaster} renderDetail={(item) => <div className="split-detail"><h3>{item.title}</h3><p>{item.detail}</p><div className="message-lines"><span /><span /><span /></div></div>} /></article></div></section>
}
...
function SettingsPage({ dark, setDark, motion, setMotion, compact, setCompact, highContrast, setHighContrast, navMode, setNavMode }: { dark: boolean; setDark: (value: boolean) => void; motion: boolean; setMotion: (value: boolean) => void; compact: boolean; setCompact: (value: boolean) => void; highContrast: boolean; setHighContrast: (value: boolean) => void; navMode: NavigationPaneMode; setNavMode: (value: NavigationPaneMode) => void }) {
  return <section className="page-enter settings-page"><header className="section-head"><h2>个性化</h2><p>NavigationView 新增 Overlay：紧凑轨道保持不动，展开窗格覆盖内容而不是推动页面。</p></header><div className="settings-stack"><ToggleSwitch checked={dark} onChange={setDark} label="深色主题" detail="切换页面明暗资源" /><ToggleSwitch checked={motion} onChange={setMotion} label="界面动画" detail="关闭非必要转场与加载动画" /><ToggleSwitch checked={compact} onChange={setCompact} label="紧凑密度" detail="提高桌面鼠标环境的信息密度" /><ToggleSwitch checked={highContrast} onChange={setHighContrast} label="高对比度" detail="模拟高对比度资源；系统 forced-colors 仍拥有更高优先级" /><div className="settings-field"><ComboBox label="NavigationView 窗格" value={navMode} onChange={(value) => setNavMode(value as NavigationPaneMode)} options={[{ value: 'auto', label: '自动' }, { value: 'compact', label: '紧凑' }, { value: 'expanded', label: '展开' }, { value: 'overlay', label: 'Overlay · 覆盖' }]} /></div></div></section>
}
